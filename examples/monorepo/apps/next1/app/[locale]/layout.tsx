...
import { I18nProviderClient } from '@/i18n'; // Client component for i18n
...
export function generateMetadata({
  params,
}: { params: { locale: Locale } }): Metadata {
  const currentLocale = params.locale;
  return {
    title: currentLocale === 'pl' ? 'Apka Next 1' : 'Next App 1',
    description: currentLocale === 'pl' 
      ...
      <body className={inter.className}>
        <I18nProviderClient locale={currentLocale as string}>
          <nav className="p-4 bg-gray-800 text-white">
            <Link href={`/${currentLocale}`} className="mr-4 hover:underline">Home</Link>
            <Link href={`/${currentLocale}/about`} className="hover:underline">About</Link>
          </nav>
          <main className="p-4">
            {children}
          </main>
