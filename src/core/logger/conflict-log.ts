...
function getVisibleLines(
  ...
  for (const errorLine of errorLines) {
    for (
      let i = Math.max(0, errorLine - contextLines);
      i <= Math.min(totalLines - 1, errorLine + contextLines);
      i++
    ) {
      visible.add(i);
    }
...
function printLines(
  ...
  errorLines: Set<number> = new Set(),
  condense: boolean = false
): void {
  const visibleLines = condense
    ? getVisibleLines(lines.length, errorLines)
    : null;
...
async function logTagConflictInfo(
  tagInfo: LangTagCLITagConflictInfo,
  prefix: string,
  conflictPath: string,
  translationArgPosition: number,
  condense?: boolean
): Promise<void> {
  const { tag } = tagInfo;
  ...
  try {
    ...
    const translationTagCode =
      translationArgPosition === 1 ? tag.parameter1Text : tag.parameter2Text;
    const configTagCode =
      translationArgPosition === 1 ? tag.parameter2Text : tag.parameter1Text;
    ...
    if (translationTagCode) {
      try {
        ...
        const translationStartInWhole =
          wholeTagCode.indexOf(translationTagCode);
        if (translationStartInWhole >= 0) {
          const linesBeforeTranslation =
            wholeTagCode.substring(0, translationStartInWhole).split('\n')
              .length - 1;
          ...
          if (translationErrorLines.size > 0) {
            ...
            lineNum =
              startLine + linesBeforeTranslation + lastTranslationErrorLine;
          }
        }
    ...
    if (configTagCode) {
      try {
        ...
        if (configStartInWhole >= 0) {
          const linesBeforeConfig =
            wholeTagCode.substring(0, configStartInWhole).split('\n').length -
            1;
...
export async function $LT_LogConflict(
