...
describe('hide-compiled-exports e2e tests', () => {
    ...
    describe('dist1 - single file with lang-tag variables', () => {
        it('should hide exports of lang-tag variables', async () => {
            ...
            try {
                ...
                expect(modifiedContent).not.toContain('export declare const testVar');
                expect(modifiedContent).toContain('declare const testVar');

                // Check that otherVar export was NOT removed (not a lang-tag)
                expect(modifiedContent).toContain('export declare const otherVar');
            } finally {
    ...
    describe('dist2 - multiple lang-tag variables', () => {
        it('should hide multiple exports of lang-tag variables', async () => {
            ...
            cpSync(join(DISTS_DIR, 'dist2'), TEST_DIST_DIR, { recursive: true });
            ...
            try {
                ...
                expect(modifiedContent).not.toContain('export declare const translations1');
                expect(modifiedContent).toContain('declare const translations1');
                expect(modifiedContent).not.toContain('export declare const translations2');
                expect(modifiedContent).toContain('declare const translations2');

                // Check that notLangTag export was NOT removed
                expect(modifiedContent).toContain('export declare const notLangTag');
            } finally {
                process.chdir(originalCwd);
            }
        });
    ...
    describe('dist3 - nested directory structure', () => {
        it('should handle nested directory structure', async () => {
            ...
            cpSync(join(DISTS_DIR, 'dist3'), TEST_DIST_DIR, { recursive: true });

            // Mock collect to return nestedVar as lang-tag variable
            vi.mocked($LT_CollectCandidateFilesWithTags).mockResolvedValue([
                createFileWithTags('src/nested/file.ts', [{ variableName: 'nestedVar' }]),
            ]);
            ...
            try {
                ...
                expect(modifiedContent).not.toContain('export declare const nestedVar');
                expect(modifiedContent).toContain('declare const nestedVar');

                // Check that regularExport was NOT removed
                expect(modifiedContent).toContain('export declare const regularExport');
            } finally {
                process.chdir(originalCwd);
            }
        });
    ...
    describe('dist4 - no matching lang-tag variables', () => {
        it('should not modify file when no lang-tag variables match', async () => {
            ...
            cpSync(join(DISTS_DIR, 'dist4'), TEST_DIST_DIR, { recursive: true });

            // Mock collect to return different variable names
            vi.mocked($LT_CollectCandidateFilesWithTags).mockResolvedValue([
                createFileWithTags('src/other.ts', [{ variableName: 'differentVar' }]),
            ]);
            ...
            try {
                ...
                expect(modifiedContent).toContain('export declare const someOtherVar');
                expect(modifiedContent).toContain('export declare const anotherVar');
            } finally {
                process.chdir(originalCwd);
            }
        });
    ...
    describe('dist5 - mixed export declarations', () => {
        it('should handle both "export declare const" and "export const"', async () => {
            ...
            cpSync(join(DISTS_DIR, 'dist5'), TEST_DIST_DIR, { recursive: true });
            ...
            try {
                ...
                expect(modifiedContent).not.toContain('export declare const langTagVar');
                expect(modifiedContent).toContain('declare const langTagVar');
                expect(modifiedContent).not.toContain('export declare const anotherLangTag');
                expect(modifiedContent).toContain('declare const anotherLangTag');
